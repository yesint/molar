...
pub trait IndexIterator: ExactSizeIterator<Item = usize> + DynClone {}
impl<T> IndexIterator for T where T: ExactSizeIterator<Item = usize> + DynClone {}

impl Clone for Box<dyn IndexIterator> {
    fn clone(&self) -> Self {
        clone_box(&*self)
    }
}
