...
impl FileFormatHandler for XtcFileHandler {
        ...
        Self: Sized,
    {
        ...
        s.handle = get_xdr_handle(fname.as_ref().to_str().unwrap(), "r")?;

        // Extract number of atoms
        let mut n_at: i32 = 0;
        let ok = unsafe { xdr_xtc_get_natoms(s.handle, &mut n_at) };
        if ok != 1 {
            // 1 is success for this function, f**ing idiots!
            return Err(XtcHandlerError::ReadNumAtoms)?;
        }

        s.natoms = n_at as usize;

        // XTC file contains step number in terms of simulation steps, not saved frames
        // So we have to extract conversion factor
        let mut ok: bool = false;
        let first_step = unsafe { xtc_get_current_frame_number(s.handle, n_at, &mut ok) };
        if !ok {
            return Err(XtcHandlerError::CantGetCurrentFrameNumber)?;
        }
        let next_step = unsafe { xtc_get_next_frame_number(s.handle, n_at) };
        if next_step < first_step {
            return Err(XtcHandlerError::CantGetNumberOfStepsPerFrame)?;
        } else if first_step == next_step {
            s.steps_per_frame = 1;
        } else {
            s.steps_per_frame = (next_step - first_step) as usize;
        }

        // Get first time
        let first_time = unsafe { xtc_get_current_frame_time(s.handle, n_at, &mut ok) };
        if !ok {
            return Err(XtcHandlerError::CantGetCurrentFrameTime)?;
        }

        // Get last frame
        let last_step = unsafe { xdr_xtc_get_last_frame_number(s.handle, n_at, &mut ok) };
        if !ok {
            return Err(XtcHandlerError::CantGetLastFrameNumber)?;
        }
        // Get last time
        let last_time = unsafe { xdr_xtc_get_last_frame_time(s.handle, n_at, &mut ok) };
        if !ok {
        ...
        if last_step < first_step || last_time < first_time {
            ...
            s.is_random_access = false;
        }

        s.frame_range = (
            first_step as usize / s.steps_per_frame,
            last_step as usize / s.steps_per_frame,
        );

        s.time_range = (first_time, last_time);

        // Get dt
        s.dt = unsafe { xdr_xtc_estimate_dt(s.handle, n_at, &mut ok) };
        if !ok {
            warn!("Can't get dt");
            s.dt = -1.0;
        }

        if !s.is_random_access {
    ...
    {
        let mut s = Self::default();
        s.handle = get_xdr_handle(fname.as_ref().to_str().unwrap(), "w")?;
        Ok(s)
    }
    ...
    fn read_state(&mut self) -> Result<State, super::FileFormatError> {
        ...
        if ok as u32 == exdrOK {
            ...
            st.pbox = Some(PeriodicBox::from_matrix(box_matrix).map_err(XtcHandlerError::Pbc)?);
        }
    ...
    fn seek_frame(&mut self, fr: usize) -> Result<(), super::FileFormatError> {
        ...
        let ret = unsafe {
            xdr_xtc_seek_frame(
                (fr * self.steps_per_frame).try_into().unwrap(),
                self.handle,
    ...
    fn seek_time(&mut self, t: f32) -> Result<(), super::FileFormatError> {
        ...
        let ret =
            unsafe { xdr_xtc_seek_time(t, self.handle, self.natoms.try_into().unwrap(), false) };
        if ret < 0 {
    ...
    fn tell_current(&self) -> Result<(usize, f32), super::FileFormatError> {
        let mut ok = false;
        let ret = unsafe {
            xtc_get_current_frame_number(self.handle, self.natoms.try_into().unwrap(), &mut ok)
        };
        if !ok || ret < 0 {
            return Err(XtcHandlerError::CantGetCurrentFrameNumber)?;
        }
        ...
        let t = unsafe {
            xtc_get_current_frame_time(self.handle, self.natoms.try_into().unwrap(), &mut ok)
        };
        if !ok || t < 0.0 {
            return Err(XtcHandlerError::CantGetCurrentFrameTime)?;
        }
    ...
    fn tell_first(&self) -> Result<(usize, f32), super::FileFormatError> {
        if !self.is_random_access {
            return Err(XtcHandlerError::RandomAccessImpossible)?;
        }
    ...
    fn tell_last(&self) -> Result<(usize, f32), super::FileFormatError> {
        if !self.is_random_access {
            return Err(XtcHandlerError::RandomAccessImpossible)?;
        }
